...
module.exports = (kota) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (!sholat.status) return resolve({ creator: '@neoxrs – Wildan Izzudin', status: false })
        let now = moment(new Date * 1).format('DD-MM-yy')
        let data = sholat.data.find(v => v.tanggal == now)
        if (typeof data == 'undefined') return resolve({ creator: '@neoxrs – Wildan Izzudin', status: false })
        resolve({ creator: '@neoxrs – Wildan Izzudin', status: true, data })
    })
}